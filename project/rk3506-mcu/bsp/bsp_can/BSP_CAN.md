...
- `CAN_Service_Init()` 用于显式初始化 CAN 服务。当前实现中，第一次 `CAN_Register()` 也会自动触发初始化。
...
- `CANRegister / CANSetDLC / CANTransmit` 是为了兼容旧命名风格保留的别名接口，功能与带下划线接口一致。
...
- `CAN_Transmit()` 会等待发送邮箱空闲，然后等待中断回写发送完成或失败状态。
