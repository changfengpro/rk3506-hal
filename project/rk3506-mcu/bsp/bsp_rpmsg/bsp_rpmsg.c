 ...
 * @brief 等待链路建立完成。
 */
static void RPMsg_WaitLinkUp(void);
...
static void RPMsg_WaitLinkUp(void)
{
    ...
    rpmsg_lite_wait_for_link_up(s_rpmsgInstance);
}
...
void RPMsg_Service_Init(void)
{
    ...
    HAL_DBG("RPMsg remote init done\n");
    HAL_DBG("instance link_id: 0x%x\n", (uint32_t)s_rpmsgInstance->link_id);
    HAL_DBG("link_state addr: 0x%x, val: %d\n",
            (uint32_t)&(s_rpmsgInstance->link_state),
            (uint32_t)s_rpmsgInstance->link_state);

    HAL_DBG("RPMsg waiting link...\n");

    RPMsg_WaitLinkUp();

    HAL_DBG("RPMsg link up! link_id-0x%x\n", (uint32_t)s_rpmsgInstance->link_id);

    s_rpmsgNsHandle = rpmsg_ns_bind(s_rpmsgInstance, RPMsg_DefaultNsBindCallback, NULL);
    HAL_ASSERT(s_rpmsgNsHandle != NULL);

    HAL_DBG("RPMsg nameservice bind done.\n");
